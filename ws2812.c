...
#define BUTTON_PIN PICO_DEFAULT_BUTTON_PIN
#else
// default to pin 15 for a button wired between the GPIO and ground
...
#if BUTTON_PIN >= NUM_BANK0_GPIOS
#error Attempting to use a button pin>=32 on a platform that does not support it
#endif
...
int main() {
    //set_sys_clock_48();
    stdio_init_all();
    printf("WS2812 Smoke Test, using pin %d; button on GPIO %d\n", WS2812_PIN, BUTTON_PIN);
    ...
    while (1) {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            if (button_event) {
                ...
                // Ignore switch bounce and additional edges from this press.
                sleep_ms(150);
                button_event = false;
            }
        }
